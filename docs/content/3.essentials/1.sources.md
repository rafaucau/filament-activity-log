...
Stamp one `batch_uuid` per save so related rows group together — via spatie's `Activity::beforeLogging` hook or `LogBatch::startBatch()` / `endBatch()`. Rows with a `null`/empty `batch_uuid` are never merged.
::
